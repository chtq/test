...
$con = Mongo::Connection.new "192.168.122.102"
$db =  $con['uclassroom']
$user = $db['dockers']
$Res  = $db['results']
$Gitpath = ""
...
class CompileRepo
	...
	def run_test_for_commits(ref, new_commits)
		...
		report_name = File.join ROOT, @name, "#{ref.commit.id}-#{Time.now.to_i}-#{ok}-#{$$}.yaml"
    ...
    if ok == "OK"
		...
		testresult = {"docker"=> $dockerid, "commits"=> commits_info, "result"=>ok, "grade" => grade,"build_time"=>Time.now, "infos"=>result}
		re_id     = $Res.insert(testresult)
	...
	$CONFIG[:repos].each do |r|
		begin
			...
			#	if doc["name"]== r[:user]
				...
				`chmod 600 ~/.ssh/id_rsa`
					#	f.puts "Host #{r[:user]}"
					#	f.puts "  HostName 192.168.0.108"
					#	f.puts "  User git"
					#	f.puts "  IdentityFile #{File.join(USERCONF, r[:user])}"
				#	end

				$Gitpath = "git@192.168.122.102:" + doc["builder"]["name"] + "/#{doc["lab"]["name"]}.git"
					#f.closeucre
		#		end
        $Studentname = doc["builder"]["name"]
				#if not FileTest::exist?(File.join(ROOT, $Studentname))
				if not repos.has_key?($Studentname)
			   repos[ doc["builder"]["name"] ] = CompileRepo.new r
				else
					next
				end
		    rescue StandardError => e
			    error "#{doc["builder"]["name"]} #{e} not available, skip"
			    puts e.backtrace
			    next
				end
			}
